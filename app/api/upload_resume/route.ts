...
export const POST = async (req: Request) => {
  try {
    ...
    if (contentType?.includes('multipart/form-data')) {
      ...
      const apitype = formData.get('inputType'); // "cv" expected
      ...
      if (apitype === 'cv') {
        ...
        const skillsJsonText = await modelStructuredOutput.generateContent([
          { text: `Take the following and extract all the skills mentioned here ${resumeParseText}` }
        ]);
        const skillsJson = JSON.parse(skillsJsonText.response.text());
        console.log('Extracted Skills:', skillsJson);

        const topSkills = await modelStructuredOutput.generateContent([
          { text: `Take the following and extract top skills which are representative of most of these. Extract some 5-10 representative skills. ${resumeParseText}` }
        ]);
      ...
      data = await req.json();
      const { name, email, skills, inputType } = data;
      ...
      if (inputType === 'manual') {
        ...
        await collection.updateOne(
          { user_id: userId },
          {
            $set: {
              name,
              email,
              skills_manual: skillList,
              'user_actions.skills_entered_manually': true,
            },
          },
          { upsert: true }
        );
        ...
        return NextResponse.json({
          message: 'Skills entered manually and saved successfully!',
        });
      }
    }
